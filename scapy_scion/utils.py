...
def capture_path(scion: str, src_br: str, sciond: str, dest: str, timeout: Optional[int] = 1,
    ...
    ### Parameters
    scion: "scion" command to use for the ping.
    src_br: Internal interface of a border router in the source AS.
        Example: "127.0.0.33:31010"
    sciond: Address of sciond in the source AS.
        Example: "127.0.0.35:30255"
    dest: Destination host in the format expected by "scion ping".
        Example: "3-ff00:0:7,127.0.0.1"
    timeout: How long to wait before giving up if no packet is captured, e.g.
        because the ping failed. The value is in seconds. None disables the timeout.
    extra_args: Additional command line arguments passed to 'scion ping'.
    capture_output: Capture the output of the 'scion ping' command and return it.
        If False, the command's output will be written to stdout and stderr as usual.

    ### Returns
    Captured SCION header with payload removed. If capture_output was True, a
    pair of the captures SCION header and the output of the ping command.

    ## Exceptions
    PingError if the 'scion ping' command failed.
    """
    ...
    cap = sniff(iface="lo", count=1, timeout=timeout,
        ...
        lfilter=lambda pkt: pkt.haslayer(SCMP) and pkt[SCMP].type==SCMP.TypeEchoRequest,
...
def compare_layers(layer1, layer2):
    ...
    Differences are returned as tuples of field name, value in `layer1` and
    value in `layer2`. Packet fields and PacketListFields are compared
    recursively.
    """
